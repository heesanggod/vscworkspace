		...
		Scanner scanner = new Scanner(System.in)) {
			...
			while (num <= n * n) {
				...
				if (x == -1) {  // 배열의 범위를 벗어나면 배열의 경계로 이동시키는 구간
					...
					// 이로써 배열의 맨 위에서 아래로 이동할 때 배열의 아래쪽 끝으로 이동하게 된다
					x = n - 1;
				}
				if (y == n) {
					y = 0;
					// 만약 y가 n이 되면(즉, 배열의 오른쪽 끝으로 이동하려고 할 때)
					// y를 0으로 설정함, 이로써 배열의 오른쪽 끝에서 왼쪽으로 이동할 때 배열의 왼쪽 끝으로 이동하게 된다
				}
				
				if (maSq[x][y] !=0) { // 현재위치에 이미 숫자가 채워져있다면, 이전 위치 (x1, y1)에서 아래로 한칸 이동하도록 한다
					x = x1 + 1; //x를 이전 위치 x1에서 아래로 한 칸 이동한 위치로 설정한다.
					// 이로써 현재 위치 (x, y)에서 아래로 한 칸 이동하게 된다
					y = y1; // y를 이전 위치 y1로 설정, 이로써 현재 위치 (x, y)에서 가로 방향은 이동하지 않게 된다
					if (x == -1) {
						x = n -1;
